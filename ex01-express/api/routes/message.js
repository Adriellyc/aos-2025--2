...
router.get("/:id", async (req, res) => {
  try {
    const message = await req.context.models.Message.findByPk(req.params.id, { include: models.User });
...
router.post("/", async (req, res) => {
  try {
    ...
    const userExists = await req.context.models.User.findByPk(userId);
    if (!userExists) {
...
router.put("/:id", async (req, res) => {
  try {
    const message = await req.context.models.Message.findByPk(req.params.id);
    if (!message) return res.status(404).json({ error: "Mensagem não encontrada" });
